...
export const getAppointmentsByService = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ ok: false, msg: 'Error obteniendo citas' });
    }
};


export const createAppointment = async ( req, res = response ) => {
    const { userId, date, sessionZones, contact, service, status } = req.body;
    try {
        ...
        const existService = await prisma.service.findUnique({ where: { id: service } });
        if (!existService) return res.status(400).json({ ok: false, msg: 'Servicio no encontrado' });

        const appointment = await prisma.appointment.create({
            data: {
                clientId: userId,
                date,
                sessionLength: existService.duration,
                sessionZones,
                contact,
                serviceId: service,
                status
...
// Adaptar esto |==> type => service (donde service es un id)
